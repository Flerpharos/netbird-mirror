...
func (zm *ZitadelManager) post(ctx context.Context, resource string, body string, headers map[string]string ) ([]byte, error) {
...
func (zm *ZitadelManager) get(ctx context.Context, resource string, q url.Values, headers map[string]string ) ([]byte, error) {
	jwtToken, err := zm.credentials.Authenticate(ctx)
	if err != nil {
		return nil, err
	}
